...
"""--- Day 6: Custom Customs --- As your flight approaches the regional 
airport where you'll switch to a much larger plane, customs declaration forms 
...
--- Part Two --- As you finish the last group's customs declaration, 
...
def main():
    ...
    # --- Part Two ---
    foo = [x.replace("\n", "") for x in data]
    print(sum([len(set(x)) for x in foo]))  # 6878
    ...
    for group in bar:
        ...
        res = reduce(lambda x, y: x.intersection(y), answers)
        total += len(res)
    print("total: ", total)  # 3464
