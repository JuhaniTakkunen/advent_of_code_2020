...
--- Part Two --- While it appears you validated the passwords correctly, 
they don't seem to be what the Official Toboggan Corporate Authentication 
System is expecting. 
...
def read_data():
    ...
    data = []
    for row in fp.read_text().splitlines():
        _, min_count, max_count, character, word, _ = re.split(regex_str, row)
        data.append((int(min_count), int(max_count), character, word))
    return data
...
def main():
    ...
    data = read_data()
    valids = 0
    for min_count, max_count, character, word in data:
        if min_count <= word.count(character) <= max_count:
            valids += 1
    print(valids)  # 666
    ...
    for min_count, max_count, character, word in data:
        try:
            first_match = word[min_count - 1] == character
        except IndexError:
            continue
        try:
            second_match = word[max_count - 1] == character
        except IndexError:
            second_match = False
